...
CREATE TABLE IF NOT EXISTS PlayerInClubSeason(
    ...
    dateOfBirth DATE NOT NULL, 
    PRIMARY KEY (playerName, clubName, season, dateOfBirth),
...
CREATE TABLE IF NOT EXISTS `Match`(
    ...
    homeRedCard int , 
    homePossessions decimal(3,1), 
    homeYellowCard int ,
    homeGoals int NOT NULL, 
    homeFouls int , 
    homeShots int , 
    awayRedCard int , 
    awayPossessions decimal(3,1) , 
    awayYellowCard int,
...
CREATE TABLE IF NOT EXISTS UserMatchReview(
    ...
    CONSTRAINT FK_homeClubName FOREIGN KEY (homeClubName) REFERENCES Club (clubName)
     ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_awayClubName FOREIGN KEY (awayClubName) REFERENCES Club (clubName)
    ON UPDATE CASCADE ON DELETE CASCADE
);
